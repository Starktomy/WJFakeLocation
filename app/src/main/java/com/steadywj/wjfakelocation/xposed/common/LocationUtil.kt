...
object LocationUtil {
    ...
    fun createFakeLocation(provider: String = "gps"): Location {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            de.robv.android.xposed.XposedHelpers.callMethod(location, "setIsFromMockProvider", true)
        }
    ...
    fun wgs84ToGcj02(
        ...
        val finalLat = lat + (dLat * 180.0) / ((6378245.0 * (1 - 0.00669342162296594323)) / (magic * sqrtMagic) * Math.PI)
        val finalLng = lng + (dLng * 180.0) / (6378245.0 / sqrtMagic * Math.cos(radLat) * Math.PI)
